...
stream has already been used up.
...
Option A seems correct, since forEach() does take a Consumer instance, but it missing a generic argument.
...
```
A. `peek(System.out::println)`  // it is never executed  and printed because peek is an intermediate operation.
B. `peek(System.out::println).findFirst()` // compile but is good practice
C. `peek(r -> System.out.println(r).findFirst()` // compile but is good practice
D. `peek(r -> {count++; System.out.println(r); }).findFirst()`
E. None of the above
F. None of these are bad practice
&emsp;&emsp;
_Answer_
D. Because it is bad practice to have a `peek()` method that has side effect like method modifying a variable.
5. Given
```java
...
after mapping it to a String. This will produce the required output.
6. Given
```java
public class Student {
    public static enum Grade {A, B, C, D, F};
...
can be determined by testing an element in the beginning itself. For example, if you inoke predicate on the given stream, 
...
tests returns truee or false. Therefore, it is clear that there ir no need for testing other elements.
9. 